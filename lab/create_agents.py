...
TOOLS = {
    ...
    "get_s3_records": {
        ...
        "description": "Read malformed records from S3 Bronze disaster folder, apply field remapping (merchant_nm→merchant_name, date format fix), and return clean records ready for loading to Snowflake.",
        "parameters": {
            "s3_prefix": {"description": "S3 prefix to read from, e.g. bronze/disaster/", "required": False, "type": "string"},
    ...
    "load_to_snowflake": {
        ...
        "description": (
            "Bulk load records to Snowflake using MERGE INTO on transaction_id (idempotent). "
            "Preferred: pass s3_staging_key + bucket from get_s3_records output. "
            "Fallback: pass records as inline JSON array."
        ),
        "parameters": {
            "s3_staging_key": {"description": "S3 key of staging file written by get_s3_records (preferred over inline records)", "required": False, "type": "string"},
            "bucket":         {"description": "S3 bucket containing the staging file", "required": False, "type": "string"},
            "records":        {"description": "JSON array of records (only if s3_staging_key not available)", "required": False, "type": "string"},
        },
    },
...
def lambda_handler(event, context):
    ...
    print(f"DISPATCHER: Invoking target Lambda function: {target}")
    lc   = boto3.client("lambda")
    resp = lc.invoke(FunctionName=target, Payload=json.dumps(event))
    payload = resp["Payload"].read().decode("utf-8")
    print(f"DISPATCHER: Target Lambda returned payload: {payload[:500]}")
    ...
    r = bedrock.create_guardrail(
        ...
        topicPolicyConfig={
            "topicsConfig": [{
                ...
                "definition": "SQL that permanently deletes database objects: DROP TABLE, DELETE FROM without WHERE, TRUNCATE TABLE. MERGE INTO, INSERT, SELECT, and data loading are allowed.",
                "examples": [
                    "DROP TABLE transactions",
                    "DELETE FROM SIGMA.SILVER.TRANSACTIONS",
                    "TRUNCATE TABLE SIGMA.SILVER.TRANSACTIONS",
                    "DROP SCHEMA SIGMA",
                ],
                "type": "DENY",
            }]
        ...
        blockedInputMessaging="Request blocked by Sigma DataTech platform guardrail. Only read/load/merge operations are permitted. DROP, DELETE without WHERE, and TRUNCATE are not allowed.",
...
def get_or_create_sub_agent(bedrock, name, dispatcher_arn, guardrail_id, account_id, role_arn):
    ...
    if existing_id:
        log(f"  Already exists: {existing_id}. Cleaning up old role-less configuration...")
        try:
            # Delete any existing aliases first to allow clean deletion
            aliases = bedrock.list_agent_aliases(agentId=existing_id).get("agentAliasSummaries", [])
            for alias in aliases:
                if alias["agentAliasName"] != "AgentTestAlias":
                    bedrock.delete_agent_alias(agentId=existing_id, agentAliasId=alias["agentAliasId"])
            bedrock.delete_agent(agentId=existing_id, skipResourceInUseCheck=True)
            
            # Wait for deletion to propagate completely (AWS best practice)
            log("  Waiting for deletion to propagate in AWS...")
            for _ in range(30):
                time.sleep(2)
                try:
                    bedrock.get_agent(agentId=existing_id)
                except bedrock.exceptions.ResourceNotFoundException:
                    break
                except Exception:
                    pass
            log(f"  Deleted old agent {name}. Recreating fresh...")
            time.sleep(3)
        except Exception as delete_err:
            log(f"  Warning: could not delete old agent {name}: {delete_err}")
...
def get_or_create_supervisor(bedrock, sub_agent_data, dispatcher_arn, guardrail_id, account_id, role_arn):
    ...
    if supervisor_id:
        log(f"  Supervisor already exists: {supervisor_id}. Cleaning up old configuration...")
        try:
            aliases = bedrock.list_agent_aliases(agentId=supervisor_id).get("agentAliasSummaries", [])
            for alias in aliases:
                if alias["agentAliasName"] != "AgentTestAlias":
                    bedrock.delete_agent_alias(agentId=supervisor_id, agentAliasId=alias["agentAliasId"])
            bedrock.delete_agent(agentId=supervisor_id, skipResourceInUseCheck=True)
            ...
            for _ in range(30):
                time.sleep(2)
                try:
                    bedrock.get_agent(agentId=supervisor_id)
                except bedrock.exceptions.ResourceNotFoundException:
                    break
                except Exception:
                    pass
            log("  Deleted old Supervisor agent. Recreating fresh...")
            time.sleep(3)
        except Exception as delete_err:
            log(f"  Warning: could not delete old Supervisor: {delete_err}")
            
    # Recreate Supervisor fresh
    r = bedrock.create_agent(
    ...
    # Create Agent Version (AWS Best Practice for Aliases)
    log("  Publishing Supervisor version...")
...
def main():
    ...
    # Pre-clean up: if SupervisorAgent exists, delete it first to break any collaborator links
    log("\nChecking for existing SupervisorAgent to break collaborator links...")
    supervisor_id = find_agent_by_name(bedrock, "SupervisorAgent")
    if supervisor_id:
        log(f"  SupervisorAgent ({supervisor_id}) exists. Deleting it first to disassociate collaborators...")
        try:
            aliases = bedrock.list_agent_aliases(agentId=supervisor_id).get("agentAliasSummaries", [])
            for alias in aliases:
                if alias["agentAliasName"] != "AgentTestAlias":
            ...
            log("  Waiting for Supervisor deletion to propagate...")
            for _ in range(30):
                time.sleep(2)
                try:
                    bedrock.get_agent(agentId=supervisor_id)
                    ...
                    pass
            log("  SupervisorAgent deleted successfully.")
        except Exception as e:
            log(f"  Warning during pre-clean up of SupervisorAgent: {e}")
